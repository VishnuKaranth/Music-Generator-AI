...
model_vol = modal.Volume.from_name("ace-step-models", create_if_missing=True)
hf_vol = modal.Volume.from_name("qwen-hf-cache", create_if_missing=True)

aws_music_gen_secrets = modal.Secret.from_name("Music-Gen-Secret")
...
class GenerateFromDescrtiptionRequest(AudioGenerationBase):
...
@app.cls(
    image=image,
    gpu = "L40s",
    volumes = {"/models": model_vol, "/.cache/huggingface": hf_vol},
    secrets = [aws_music_gen_secrets],
    scaledown_window=15
)
class MusicGenServer:
    ...
    def load_modal(self):
        ...
        # Stable Diffusion Model (thumbnailes)
    ...
    def generate_prompt(self, description: str):
        ...
        # Run LLM ingerence and return that
    ...
    def generate_and_upload_to_s3(
        ...
        # AWS
        # Create S3 Bucket: thumnails, songs
        # IAM Users:
    ...
    @modal.fastapi_endpoint(method="POST", requires_proxy_auth=True)
    def generate_from_description(self, request: GenerateFromDescrtiptionRequest) -> GenerateMusicResponseS3:
        # Generating a prompt
...
def main():
    ...
    request_data = GenerateWithDescribedLyricsRequest(
        prompt="drum and bass, electro house",
        described_lyrics="Cyberpunk and the future",
        guidance_scale=15
    )
    ...
    print(f"Sucess: {result.s3_key} {result.cover_image_s3_key} {result.categories}")
    # audio_data = base64.b64decode(result.audio_data)
    # output_filename = "gen.wav"
